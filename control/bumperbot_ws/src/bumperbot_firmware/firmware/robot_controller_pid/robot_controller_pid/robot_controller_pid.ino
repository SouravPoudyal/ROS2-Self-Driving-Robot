...
// Initialize PID controller (gains, output limits)
PID right_motor(Kp_r, Ki_r, Kd_r, 0, 180);
PID left_motor(Kp_l, Ki_l, Kd_l, 0, 180);
...
void setup() {
  ...
  right_motor.setMode(true);
  // Set the mode to automatic
  left_motor.setMode(true);
}
...
void loop() {
  ...
  if (current_millis - last_millis >= interval) {
    ...
    right_wheel_cmd = right_motor.compute(right_wheel_cmd_vel, right_wheel_meas_vel, deltaT);
    left_wheel_cmd = left_motor.compute(left_wheel_cmd_vel, left_wheel_meas_vel, deltaT);
