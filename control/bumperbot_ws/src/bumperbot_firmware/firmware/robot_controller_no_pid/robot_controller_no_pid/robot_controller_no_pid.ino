...
unsigned int max_pwm = 140;

// Declare variables for the low-pass filter
double vFiltRight = 0.0, vPrevRight = 0.0;
double vFiltLeft = 0.0, vPrevLeft = 0.0;

// Low-pass filter parameters (cutoff frequency at 25 Hz)
const double alpha = 0.854;  // Coefficient for the previous filtered value
const double beta = 0.0728; // Coefficient for the measured velocity

// Apply the low-pass filter to the measured velocities
void applyLowPassFilter(double& filteredVel, double& prevVel, double measVel) {
    filteredVel = alpha * filteredVel + beta * measVel + beta * prevVel;
    prevVel = measVel; // Update the previous velocity
}
...
void loop() {
  ...
  if (current_millis - last_millis >= interval) {
    ...
    double right_wheel_meas_vel = (10 * right_encoder_counter * (60.0 / 385.0)) * 0.10472;
    double left_wheel_meas_vel = (10 * left_encoder_counter * (60.0 / 385.0)) * 0.10472;

    // Apply the low-pass filter to the measured velocities
    applyLowPassFilter(vFiltRight, vPrevRight, right_wheel_meas_vel);
    applyLowPassFilter(vFiltLeft, vPrevLeft, left_wheel_meas_vel);

    // Map velocities to PWM
    int right_wheel_cmd  = map(constrain(abs(right_wheel_cmd_vel), 0, 1), 0, 1, 0, max_pwm);
    int left_wheel_cmd = map(constrain(abs(left_wheel_cmd_vel), 0, 1), 0, 1, 0, max_pwm);
    ...
    // Synchronization logic: Adjust the faster wheel
    float speed_diff = right_wheel_meas_vel - left_wheel_meas_vel;
    if (abs(speed_diff) > 2) {  // Allow small differences as tolerance
      if (speed_diff > 0) {
        // Right wheel is faster
        right_wheel_cmd -= speed_diff * 2; // Scale adjustment as needed
        if (right_wheel_cmd < 0) right_wheel_cmd = 0; // Prevent negative PWM
      } else {
        // Left wheel is faster
        left_wheel_cmd += speed_diff * 2; // Scale adjustment as needed
        if (left_wheel_cmd < 0) left_wheel_cmd = 0; // Prevent negative PWM
      }
    }
