...
{
public:
    SimpleServiceClient(int a, int b) : Node("simple_service_server")
    {
        ...
        while(!client_->wait_for_service(1s))
        {
            ...
            RCLCPP_INFO(get_logger(), "Service not availabel, waiting again....");
        }
...
private:
    ...
    void responseCallback(rclcpp::Client<bumperbot_msgs::srv::AddTwoInts>::SharedFuture future)
    {
        if(future.valid())
        {
            ...
            RCLCPP_INFO_STREAM(get_logger(), "Service Failed");
        }
    }
