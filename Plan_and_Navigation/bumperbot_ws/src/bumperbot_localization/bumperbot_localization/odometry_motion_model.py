...
class OdometryMotionModel(Node):
    ...
    def __init__(self):
        ...
        if self.nr_samples >= 0:
            ...
            self.get_logger().fatal('Invalid Number of Samples requested: %d! Ecit..', self.nr_samples)
            return


        self.odom_sub = self.create_subscription(Odometry, "bumperbot_controller/odom", self.odomCallback, 10)
        self.pose_array_pub = self.create_publisher(PoseArray, "odometry_motion/samples", 10)
    ...
    def odomCallback(self, odom):
        ...
        delta_trasl = sqrt(pow(odom_x_increment, 2) + pow(odom_y_increment, 2))
